...
[![Typed](https://img.shields.io/badge/mypy-clean-blue.svg)](pyproject.toml)
...
<p align="center">
  <img src="docs/figures/sod_evolution.gif" width="820" alt="Sod shock tube evolving in time">
</p>

<p align="center">
  <em>Sod's shock tube: a rarefaction fan runs left, a contact and a shock run right.
...
| **`notebooks/`** | [`demo.ipynb`](notebooks/demo.ipynb) — a guided tour of the package, executed and rendered — plus the original exploratory notebooks. |
...
Prefer reading to running? [`notebooks/demo.ipynb`](notebooks/demo.ipynb) walks through
the whole package with its outputs already rendered — no clone needed.
...
The code is checked, not just run. `pytest` covers **133 tests**, all of which assert a
...
```bash
make test       # pytest with coverage (fails below 95%)
make lint       # ruff
make types      # mypy, clean over the whole package
make results    # regenerate every figure and table in docs/
make animation  # regenerate the GIF above
make notebook   # re-execute the demo notebook
```

CI runs lint, mypy and the suite on Python 3.10, 3.11 and 3.12; a second job re-derives
`docs/RESULTS.md` and fails if the committed numbers have gone stale; a third re-executes
the demo notebook so it cannot drift from the code.
...
├── tests/                 # 133 property-based tests
├── scripts/               # generate_results.py, make_animation.py, build_demo_notebook.py
├── docs/                  # RESULTS.md + figures
├── report/                # LaTeX sources, report.pdf, slides
├── matlab/                # original MATLAB prototype
└── notebooks/             # demo.ipynb (Python) + the original MATLAB notebooks
```
...
| Verification | visual comparison against the exact solution | 133 automated tests + a convergence study in CI |
